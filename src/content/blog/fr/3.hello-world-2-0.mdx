...
### Pages
Chaque page utilise un layout situé dans `src/layouts/Layout.astro` qui définit les éléments communs à toutes les pages.
...
### Posts
Les fichiers `*.mdx` sont utilisés par le fichier `src/pages/[lang]/blog/[slug].astro` où `slug` correspond au nom du fichier `*.mdx` slugifié. Ces fichiers sont répartis en deux dossiers `fr` et `en` situés dans `src/content/blog/` contenant chacun les versions françaises et anglaises des mêmes articles.
...
4. une intégration copie en fin de build le contenu de ce répertoire dans le dossier de build ([parce qu'Astro ne permet pas à une page de rendre un deuxième fichier](https://github.com/withastro/roadmap/discussions/643)).
