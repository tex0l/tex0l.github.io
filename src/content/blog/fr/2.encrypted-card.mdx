...
Vous avez peut-être déjà vu ces cartes de visite réutilisables que l'on ne donne pas, mais que l'on fait scanner (par QRCode ou NFC), et qui permettent de télécharger directement la fiche contact.
...
L'objectif de cet article est de faire un équivalent de ce service, mais avec un lien qui pointe vers ce site statique, sans pour autant que n'importe qui parcourant ce site ne puisse y avoir accès.

TLDR: j'explique comment j'ai codé une carte de visite chiffrée <Icon name="noto:man-shrugging" class="inline w-6"/>.

## Fonctionnement d'une carte de visite NFC ou QRCode
Lors d'un salon récemment, j'ai rencontré quelqu'un qui avait une carte en bois sans rien écrit dessus, il a tapé l'arrière de mon téléphone avec sa carte et cela m'a proposé d'ouvrir un lien avec sa carte de visite en ligne, depuis laquelle j'ai pu directement importer le contact. J'ai trouvé ça extraordinairement pratique.

Le fonctionnement est simple, il s'agit d'une carte contenant une puce RFID qui contient un lien, et la fonctionnalité NFC de mon téléphone permet de lire ladite puce. Ce lien pointe vers un site que le fabricant (comme [MyEverCard](https://myevercard.com/) par exemple) met à disposition pour que l'on puisse y ajouter ses informations de contact, et cela donne une sorte de carte de visite en ligne.

L'idée est drôle, mais je préfèrerais ne pas faire reposer l'hébergement de mes coordonnées sur un service tiers à qui je paierais un abonnement, et même sans, celui-ci pourrait tomber en panne, cesser son activité, ou simplement faire fuiter mes données...

L'hébergement statique naïf n'est pas une option, tout le monde aurait accès sans filtre à mon numéro de téléphone, et déjà que je reçois beaucoup d'appels de prospection non sollicités, ce serait tendre le baton pour se faire battre.

Pour pallier ce problème, j'ai imaginé deux options :
- soit héberger un serveur web qui aurait à froid mes coordonnées, mais qui ne les afficherait que si un jeton d'authentification contenu dans le lien inséré dans la carte de visite est présent.
- soit chiffrer les données, héberger les données chiffrées sur un serveur statique, et déchiffrer les données côté client.

Si vous connaissez mon amour pour la gestion de serveurs (qui n'est égalé que par mon amour pour les piqûres de moustique), vous pourrez comprendre pourquoi j'ai choisi la deuxième option. Aussi, cela permet de le faire gratuitement avec [ce site hébergé sur Github Pages](../hello-world/).
...
@param hideGroupType true
    ...
    rectangle "Carte de visite" {
        [https&colon;\/\/tex0l.github.io\/fr\/encrypted-card\/#clé] as KEY
    }
...
2. La connexion TLS servant le code statique qui pourrait être compromise pour permettre d'altérer le code du site statique.
...
Pour dériver une clé de 32 bytes à partir de ce mot de passe, j'utilise la fonction de dérivation `scrypt` et un sel aléatoire accessible statiquement dans un fichier `/salt`.
...
const salt = Buffer.from(await readFile('./public/salt'), 'base64')
...
const encryptFile = async (data: Buffer, key: Buffer): Buffer => {
  const nonce: Buffer = randomBytes(12) // Je génère un nonce aléatoire de 12 octets
  const cipher: Cipher  = createCipheriv('aes-256-gcm', key, nonce)  // J'instancie un objet Cipher avec la clé de 256 bits et le nonce pour l'algorithme AES-GCM
  const encryptedData: Buffer = Buffer.concat([cipher.update(data), cipher.final()]) // Je chiffre la donnée

  const authTag: Buffer = cipher.getAuthTag() // Je calcule le MAC

  return Buffer.concat([iv, encryptedData, authTag]) // Je concatène l'IV, la donnée chiffrée et le MAC
}
```
...
const salt = await (await window.fetch('/salt')).text() // le sel est disponible publiquement
...
À partir de ces versions brutes des données déchiffrées que l'on a sous forme de `Blob`, on peut alors les utiliser dans le contexte de la page :
...
En revanche, voici une version "dummy" que vous pouvez essayer: [https://tex0l.github.io/fr/encrypted-card-dummy#my-super-secret-password](/encrypted-card-dummy#my-super-secret-password)
