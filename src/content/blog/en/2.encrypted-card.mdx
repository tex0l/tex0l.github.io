...
flowchart TB
    ...
    subgraph GH["GitHub"]
        G[Encrypted contact info]
    end

    subgraph Card["Business card"]
        KEY["tex0l.github.io/en/encrypted-card/#key"]
    end

    subgraph Browser["My contact's browser"]
        K[Encrypted contact info]
        KEYBROWSER[Key]
        L[Decryption]
        M[Contact info]
        N[Display]
...
// This function is called by `encryptDir` for each file to encrypt
